...
function BarList({ rows, isDark, formatValue = duration }: { rows: Array<{ id?: number; name: string; subtitle?: string; value: number; detail: string }>; isDark: boolean; formatValue?: (value: number) => string }) {
  const { styles } = useScreenStyles();
  const max = Math.max(1, ...rows.map(row => row.value));
  return <View style={{ gap: 13 }}>
    {rows.length ? rows.map(row => <View key={row.id ?? row.name}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
        <Text numberOfLines={2} style={[styles.rowTitle, { flex: 1 }, isDark && styles.textPrimaryDark]}>{row.name}</Text>
        <Text style={[styles.meta, isDark && styles.textMutedDark]}>{formatValue(row.value)}</Text>
      </View>
      {!!row.subtitle && <Text numberOfLines={2} style={[styles.meta, isDark && styles.textMutedDark]}>{row.subtitle}</Text>}
...
export function ReadingStatsScreen({ back }: { back: () => void }) {
  ...
  return <Screen scroll>
      ...
      {error ? <View><Text style={{ color: tokens.colors.danger }}>{error}</Text><Button label="重新加载统计" onPress={() => setAttempt(n => n + 1)} /></View> : loading || !summary ? <View style={uiStyles.loadingState}><ActivityIndicator color={tokens.colors.primary} /><Text style={[styles.meta, isDark && styles.textMutedDark]}>正在计算阅读数据…</Text></View> : <>
        ...
        <View style={[styles.statsPanel, isDark && styles.cardDark]}><Text style={[styles.sectionTitle, isDark && styles.textPrimaryDark]}>按作品</Text><BarList isDark={isDark} rows={summary.byBook.map(row => ({ id: row.id, name: row.title, subtitle: row.chapterTitle, value: row.durationMs, detail: `${row.pages} 页 · ${Math.round(row.progress * 100)}%`, }))} /></View>
