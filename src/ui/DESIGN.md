...
- Spacing: 4/8/12/16/24/32. Cover radius 6; ordinary containers 12; sheets 20. Home shelf covers follow the source image aspect ratio at equal column widths, without cropping or letterboxing; missing/failed covers use a 2:3 title fallback. Compact entries and detail thumbnails retain their existing contain frames.
...
- Navigation: 书架 / 分类 / 最近 / 设置. Preserve route keys and data. Management actions belong in labeled menus. Recent reading starts with a compact Monday-to-Sunday chart, followed by history. Group settings by purpose; choices open a selected-value panel.
- Reuse Screen, ScreenHeader, BookCover, BookCard, SettingsRow/Group, ChoiceField, BottomSheet and reading summaries. Callers own persistence. Keep virtualized grids and chapter lists, with column-dependent keys. Home uses MasonryFlashList 1.6.4 (the Expo 51 bundled compatibility version); columns flow independently without entrance animations on recycled cards.
