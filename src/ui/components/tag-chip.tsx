...
export function TagChip({ label, author = false, selected = false, onPress, onRemove, locked = false }: {
  ...
  const foreground = selected ? tokens.colors.onSelectedContainer : tokens.colors.mutedText;
  const content = <><Ionicons name={author ? 'person-outline' : 'pricetag-outline'} size={16} color={foreground} /><Text numberOfLines={2} style={[tokens.typography.caption, { color: foreground, flexShrink: 1 }]}>{label}</Text>{locked && <Ionicons name="lock-closed-outline" size={12} color={foreground} />}</>;
