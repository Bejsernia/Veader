...
export function BookCard({ title, author, coverUri, progress, footer, index = 0, onPress }: Props) {
  ...
  return <Animated.View entering={reducedMotion ? undefined : FadeInUp.delay(Math.min(index, 5) * tokens.motion.stagger).duration(tokens.motion.normal)} style={{ flex: 1, minWidth: 0, marginBottom: tokens.spacing.lg }}>
