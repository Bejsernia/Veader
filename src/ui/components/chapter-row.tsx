...
export function ChapterRow({ number, title, meta, progress, current = false, onPress }: Props) {
  ...
  return <PressableScale accessibilityRole="button" accessibilityLabel={'打开第 ' + number + ' 章：' + title} accessibilityState={{ selected: current }} haptic="light" onPress={onPress} disabled={!onPress} style={{ minHeight: 68, flexDirection: 'row', alignItems: 'center', gap: tokens.spacing.md, paddingVertical: tokens.spacing.sm, borderBottomWidth: 0.5, borderBottomColor: tokens.colors.divider }}>
    <View style={{ width: 40, height: 40, borderRadius: tokens.radius.md, alignItems: 'center', justifyContent: 'center', backgroundColor: current ? tokens.colors.primary : tokens.colors.elevated }}>
      <Text style={{ color: current ? tokens.colors.onPrimary : tokens.colors.text, fontSize: 12, fontWeight: '600' }}>{number}</Text>
    </View>
    ...
    {progress !== undefined ? <View style={{ width: 54, gap: 4 }}><Text style={{ color: tokens.colors.mutedText, fontSize: 11, textAlign: 'right' }}>{Math.round(progress * 100)}%</Text><ProgressBar value={progress} color={tokens.colors.primary} height={4} /></View> : null}
  </PressableScale>;
}
