export function getGridLayout(width: number, fontScale = 1) {
  ...
  const defaultColumns = width < 600 ? 2 : Math.min(6, Math.floor((contentWidth - baseInset * 2 + gutter) / (120 + gutter)));
  const columns = Math.max(1, Math.floor(defaultColumns / Math.max(1, fontScale / 1.3)));
